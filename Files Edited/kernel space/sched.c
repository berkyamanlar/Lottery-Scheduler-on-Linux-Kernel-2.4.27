...
#include <linux/getticket.h> // phase 2
#include <linux/setticket.h> // phase 2
#include <linux/random.h>

// Define constants for time conversions // phase 2
#define msecs_to_jiffies(ms) ((ms) * HZ / 1000)
#define TICKET_DECAY_MS 20
#define TICKET_INCREASE_MS 100
...
asmlinkage void schedule(void)
{
    ...
    int this_cpu, c;
    unsigned long now = jiffies;  // Get the current time in jiffies
...
need_resched_back:
    ...
    // Initialize ticket to 5 if not already initialized
    if (current->ticket == 0) {
        current->ticket = 5;
    }

    // Ticket adjustment logic based on time elapsed since last on CPU
    list_for_each(tmp, &runqueue_head) {
        p = list_entry(tmp, struct task_struct, run_list);
        unsigned long elapsed = now - p->last_cpu_time;

        if (elapsed <= msecs_to_jiffies(20)) {
            if (p->ticket > 1) {
                p->ticket--;
            }
        } else if (elapsed > msecs_to_jiffies(100)) {
            if (p->ticket < 10) {
                p->ticket++;
            }
        }
...
repeat_schedule:
    ...
    // Update the last_cpu_time for the selected task
    next->last_cpu_time = now;
